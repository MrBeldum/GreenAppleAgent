...
- In autonomous runs, never write temporary dispatcher/log/requeue output under `/tmp`, `/var`, or any other external directory. Keep scratch files under the exact active `$DIR` (for example `$DIR/tmp.operator/dispatcher_requeue_<case>.out`) so OpenCode never raises an `external_directory` permission prompt for unattended bookkeeping.
