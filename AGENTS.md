...
## Scope
- This repo ships an OpenCode-only agent runtime for ParrotOS/Kali VMs against authorized Hack The Box/CTF machines.
...
## Runtime
- Install with `./install.sh opencode ~/greenapple-agent`, start with `./run-htb.sh`, then run `/htb 10.10.x.x`.
...
- `./scripts/htb_preflight.sh <target>` checks required tools, HTB VPN hints, and quick reachability.
